...
This is my file that implements my variables and methods for the ClassicMode class.
*/
...
int ClassicMode::checkNeighbors(int r, int c, GameBoard* g){
  int rows = g->getRows();
  int cols = g->getCols();
  int count = 0;
  char** grid = g->getGrid();
...
GameBoard* ClassicMode::CreateNextGen(GameBoard* g){
  int rows = g->getRows();
  int cols = g->getCols();
